...
export function FocusRail({
  ...
  // Guard: avoid wrap(0,0,...) crash if items is empty
  if (count === 0) {
    return (
      <div
        className={cn(
          "relative flex h-[600px] w-full items-center justify-center overflow-hidden bg-neutral-950 text-white",
          className
        )}
      >
        <p className="text-sm text-neutral-400">No items provided.</p>
      </div>
    );
  }
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                ...
                animate={{
                  x: xOffset,
                  // NOTE: framer-motion supports z in many versions, but TS can complain in some setups.
                  // If you get a TS error on `z`, remove this line OR use transform-based approach.
                  z: zOffset as unknown as number,
                  scale,
                  rotateY,
                  opacity,
                  filter: `blur(${blur}px) brightness(${brightness})`,
                }}
                // ✅ TS-safe transition object (fixes your transition error)
                transition={{
                  x: BASE_SPRING,
                  z: BASE_SPRING,
                  rotateY: BASE_SPRING,
